...
"""This is a Prometheus exporter for Pulp."""
...
from pulp_metrics_exporter.jobs import REGISTERED_JOBS
import pulp_metrics_exporter.jobs as jobs

init_logging()


_LOGGER = logging.getLogger("pulp_metrics_exporter")
_LOGGER.info("Pulp Metrics Exporter v%s", __service_version__)

_UPDATE_INTERVAL_SECONDS = int(os.getenv("PULP_METRICS_EXPORTER_UPDATE_INTERVAL", 20))
_GRAFANA_REDIRECT_URL = os.getenv("PULP_METRICS_EXPORTER_GRAFANA_REDIRECT_URL", "https://grafana.datahub.redhat.com/")
_MAX_WORKERS = int(os.getenv("PULP_METRICS_EXPORTER_MAX_WORKERS", 16))

_INITIALIZED = bool(int(os.getenv("PULP_METRICS_EXPORTER_WEAK_READINESS", 0)))
...
application = Flask("pulp_metrics_exporter")
...
def extend_response_headers(response):
    ...
    response.headers["X-Pulp-Metrics-Exporter-Version"] = f"v{__service_version__}"
    return response
